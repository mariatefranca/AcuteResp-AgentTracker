...
# MAGIC
# MAGIC %pip install --force-reinstall databricks-feature-engineering
...
fe.write_table(name=user_preference_feature_table, df=user_df)
...
# Create some dummy data
hotel_data = [('AB123', 'Hotel Miramalfi',  340.0, 0.05),
        ('SW345',  'Hotel Marina Riviera', 442.0, 0.07),
        ('MJ564', 'Hotel Luna Convento', 566.0, 0.03),
        ('QE278',  'Hotel Residence', 450.0, 0.02)]

# Create a DataFrame with the dummy data
hotel_df = spark.createDataFrame(hotel_data, schema=hotel_schema)
fe.write_table(name=hotel_prices_feature_table, df=hotel_df)

# COMMAND ----------

# MAGIC %md ### Sync the feature tables to a Databricks online table
# MAGIC
# MAGIC Databricks online tables are managed tables to provide low-latency lookup of your data in ML serving solutions. The following code uses the Databricks SDK to create an online table.  
# MAGIC
# MAGIC You can also use the Databricks UI to create the online table. For more information, see the Databricks online tables documentation ([AWS](https://docs.databricks.com/en/machine-learning/feature-store/online-tables.html) | [Azure](https://docs.microsoft.com/azure/databricks/machine-learning/feature-store/online-tables)).
...
# online_user_table_pipeline = w.online_tables.create(name=f"{catalog_name}.{schema_name}.user_prefs_online", spec=user_spec)

# # Create hotel prices online table
# hotel_spec = OnlineTableSpec(
#   primary_key_columns=["hotel_id"],
#   source_table_full_name=hotel_prices_feature_table,
#   run_triggered=OnlineTableSpecTriggeredSchedulingPolicy.from_dict({'triggered': 'true'})
# )

# online_hotel_table_pipeline = w.online_tables.create(name=f"{catalog_name}.{schema_name}.hotel_prices_online", spec=hotel_spec)
...
user_endpoint_name = f"{schema_name}-user-preferences"
...
hotel_endpoint_name = f"hotel-price"
...
class UserPreferenceTool(BaseTool):
    name: str = "User Preference Feature Server"
    description: str = "Use this tool when you need to fetch current users travel preferences."
...
class TotalPriceTool(BaseTool):
    name: str = "Total vacation price Feature Server"
    description: str = "Use this tool when you need to fetch hotels total price including taxes."
...
# MAGIC %pip install --upgrade langchain langchain-community langchain-openai
